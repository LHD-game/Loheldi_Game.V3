...
public class NPCRandomMove : MonoBehaviour
{
    ...
    Rigidbody rigid;
    public int nextMove1;//다음 행동지표를 결정할 변수
    ...
    void FixedUpdate()
    {
        ...
        if (!raycastbool)
        {
            ...
            nextMove1 = nextMove1 * (-1); //우리가 직접 방향을 바꾸어 주었으니 Think는 잠시 멈추어야함
            nextMove2 = nextMove2 * (-1); //우리가 직접 방향을 바꾸어 주었으니 Think는 잠시 멈추어야함
